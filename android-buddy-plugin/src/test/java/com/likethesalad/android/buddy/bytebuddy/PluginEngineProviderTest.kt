...
class PluginEngineProviderTest : BaseMockable() {
    ...
    @MockK
    lateinit var androidBuddyEntryPoint: AndroidBuddyEntryPoint
    ...
    fun setUp() {
        pluginEngineProvider = spyk(
            PluginEngineProvider(
                instantiator, transformationLogger, androidBuddyEntryPoint, logger
            )
        )
    ...
    fun `Make engine with logger and android java class target version`() {
        ...
        every {
            instantiator.makePluginEngineOf(androidBuddyEntryPoint, classFileVersion, methodNameTransformer)
